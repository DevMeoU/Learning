...
{
    ESP_LOGI(TAG, "Reading scan.html");

    // Open for reading scan.html
    FILE* f = fopen("/spiffs/scan.html", "r");
    if (f == NULL) {
        ESP_LOGE(TAG, "Failed to open scan.html");
        return;
    }
    ...
    ESP_LOGI(TAG, "Read from scan.html: %s", buf);
}
...
void app_main(void)
{
    ...
    // Read and display the contents of a small text file (scan.html)
    read_hello_txt();
